# DataFusion 文档中心

欢迎来到 DataFusion 的文档中心！这里包含了项目的所有文档。

## 📚 文档导航

### 🚀 快速开始

| 文档 | 说明 | 适合人群 |
|------|------|---------|
| [../QUICK_START_GUIDE.md](../QUICK_START_GUIDE.md) | 5 分钟快速开始指南 | 新用户 |

### 🏗️ 架构和实现

| 文档 | 说明 | 适合人群 |
|------|------|---------|
| [WORKER_IMPLEMENTATION.md](WORKER_IMPLEMENTATION.md) | Worker 实现说明 | 开发者 |
| [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) | 项目结构说明 | 开发者 |

### ☸️ Kubernetes 部署

| 文档 | 说明 | 适合人群 |
|------|------|---------|
| [K8S_DEPLOYMENT_GUIDE.md](K8S_DEPLOYMENT_GUIDE.md) | K8S 完整部署指南 | 运维人员 |

### 🔧 专业指南

| 文档 | 说明 | 适合人群 |
|------|------|---------|
| [CONTROL_PLANE_API.md](CONTROL_PLANE_API.md) | 完整的 REST API 文档 | 开发者 |
| [WEB_RPA_GUIDE.md](WEB_RPA_GUIDE.md) | Web RPA 采集器配置指南（含登录/会话/动态交互） | 用户/开发者 |
| [DATABASE_COLLECTOR_GUIDE.md](DATABASE_COLLECTOR_GUIDE.md) | 数据库采集指南 | 开发者 |

### 🐛 问题修复

| 文档 | 说明 | 适合人群 |
|------|------|---------|
| [QUICK_FIX.md](QUICK_FIX.md) | 常见问题快速修复 | 运维人员 |
| [FIX_DUPLICATE_KEY_ISSUE.md](FIX_DUPLICATE_KEY_ISSUE.md) | 主键冲突详细分析 | 开发者 |

### 📁 归档文档

| 目录 | 说明 |
|------|------|
| [archive/design/](archive/design/) | 设计阶段文档 |
| [archive/history/](archive/history/) | 历史开发文档 |

## 🎯 按角色查找文档

### 👨‍💻 开发者
- [WORKER_IMPLEMENTATION.md](WORKER_IMPLEMENTATION.md) - 了解 Worker 实现
- [CONTROL_PLANE_API.md](CONTROL_PLANE_API.md) - API 接口文档
- [WEB_RPA_GUIDE.md](WEB_RPA_GUIDE.md) - Web RPA 采集配置（登录/会话/动态交互）
- [DATABASE_COLLECTOR_GUIDE.md](DATABASE_COLLECTOR_GUIDE.md) - 数据库采集
- [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) - 项目结构

### 🚀 运维人员
- [K8S_DEPLOYMENT_GUIDE.md](K8S_DEPLOYMENT_GUIDE.md) - K8S 部署
- [QUICK_FIX.md](QUICK_FIX.md) - 问题修复

### 👤 新用户
- [../QUICK_START_GUIDE.md](../QUICK_START_GUIDE.md) - 快速开始
- [../README.md](../README.md) - 项目概览

## 📖 文档维护

本文档中心定期更新，确保信息的准确性和完整性。如果发现问题或需要补充，请提交 Issue。

---

**文档总数**: 8 个核心文档  
**最后更新**: 2025-01-25

**第一次使用**：
1. [QUICKSTART.md](QUICKSTART.md) - 快速开始
2. [GETTING_STARTED.md](GETTING_STARTED.md) - 详细入门
3. [VERIFICATION_SUCCESS.md](VERIFICATION_SUCCESS.md) - 验证功能

**深入了解**：
1. [WORKER_IMPLEMENTATION.md](WORKER_IMPLEMENTATION.md) - 实现细节
2. [IMPLEMENTATION_SUMMARY.md](IMPLEMENTATION_SUMMARY.md) - 实现总结
3. [FILES_CREATED.md](FILES_CREATED.md) - 文件清单

**问题排查**：
1. [FIX_DUPLICATE_KEY_ISSUE.md](FIX_DUPLICATE_KEY_ISSUE.md) - 常见问题

### 🔧 运维人员

**Kubernetes 部署**：
1. [K8S_QUICK_START.md](K8S_QUICK_START.md) - 2 步快速部署
2. [K8S_DEPLOYMENT_GUIDE.md](K8S_DEPLOYMENT_GUIDE.md) - 完整部署指南
3. [K8S_FILES_SUMMARY.md](K8S_FILES_SUMMARY.md) - 文件说明

**问题修复**：
1. [QUICK_FIX.md](QUICK_FIX.md) - 快速修复
2. [FIX_DUPLICATE_KEY_ISSUE.md](FIX_DUPLICATE_KEY_ISSUE.md) - 详细分析

### 📋 项目管理者

**项目状态**：
1. [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) - 项目概览
2. [PROJECT_STATUS.md](PROJECT_STATUS.md) - 项目状态
3. [SUMMARY.md](SUMMARY.md) - 项目总结

**实现情况**：
1. [IMPLEMENTATION_SUMMARY.md](IMPLEMENTATION_SUMMARY.md) - 实现总结
2. [FILES_CREATED.md](FILES_CREATED.md) - 交付清单

## 📖 按主题查找文档

### 快速开始

- [QUICKSTART.md](QUICKSTART.md) - 5 分钟快速开始
- [GETTING_STARTED.md](GETTING_STARTED.md) - 10 分钟详细入门
- [K8S_QUICK_START.md](K8S_QUICK_START.md) - K8S 快速部署

### 部署指南

- [K8S_DEPLOYMENT_GUIDE.md](K8S_DEPLOYMENT_GUIDE.md) - K8S 完整部署
- [K8S_FILES_SUMMARY.md](K8S_FILES_SUMMARY.md) - K8S 文件说明

### 开发文档

- [WORKER_IMPLEMENTATION.md](WORKER_IMPLEMENTATION.md) - Worker 实现
- [IMPLEMENTATION_SUMMARY.md](IMPLEMENTATION_SUMMARY.md) - 实现总结
- [FILES_CREATED.md](FILES_CREATED.md) - 文件清单

### 验证和测试

- [VERIFICATION_SUCCESS.md](VERIFICATION_SUCCESS.md) - 验证报告

### 问题修复

- [QUICK_FIX.md](QUICK_FIX.md) - 快速修复
- [FIX_DUPLICATE_KEY_ISSUE.md](FIX_DUPLICATE_KEY_ISSUE.md) - 详细分析

### 项目信息

- [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) - 项目概览
- [PROJECT_STATUS.md](PROJECT_STATUS.md) - 项目状态
- [SUMMARY.md](SUMMARY.md) - 项目总结

## 🔍 文档搜索

### 按关键词

- **快速开始**: QUICKSTART.md, GETTING_STARTED.md, K8S_QUICK_START.md
- **部署**: K8S_DEPLOYMENT_GUIDE.md, K8S_FILES_SUMMARY.md
- **实现**: WORKER_IMPLEMENTATION.md, IMPLEMENTATION_SUMMARY.md
- **验证**: VERIFICATION_SUCCESS.md
- **修复**: QUICK_FIX.md, FIX_DUPLICATE_KEY_ISSUE.md
- **Kubernetes**: K8S_*.md
- **问题**: FIX_*.md, QUICK_FIX.md

## 📝 文档更新日志

| 日期 | 文档 | 更新内容 |
|------|------|---------|
| 2026-02-25 | WEB_RPA_GUIDE.md | 新增 Cookie 注入章节（短信/扫码验证场景） |
| 2026-02-25 | WEB_RPA_GUIDE.md | 新增 Web RPA 采集配置指南（登录/会话/动态交互） |
| 2026-02-25 | WORKER_IMPLEMENTATION.md | 更新 RPA 采集器能力说明 |
| 2025-12-04 | FIX_DUPLICATE_KEY_ISSUE.md | 新增主键冲突修复文档 |
| 2025-12-04 | QUICK_FIX.md | 新增快速修复指南 |
| 2025-12-04 | K8S_*.md | 新增 K8S 部署文档 |
| 2025-12-04 | VERIFICATION_SUCCESS.md | 新增验证报告 |
| 2025-12-04 | WORKER_IMPLEMENTATION.md | 新增实现说明 |

## 🤝 贡献文档

欢迎贡献文档！请遵循以下规范：

1. 使用 Markdown 格式
2. 添加清晰的标题和目录
3. 包含代码示例
4. 更新本 README 的索引

## 📧 反馈

如果文档有任何问题或建议，请：
1. 提交 Issue
2. 发起 Pull Request
3. 联系项目维护者

---

**文档是项目的重要组成部分！** 📚
